<?php
    ...
    $total_sales_query = "SELECT COUNT(*) as total_sales FROM transactions WHERE status = 'success' AND type NOT IN ('wallet topup', 'wallet deduction') AND DATE(created_at) = CURDATE()";
    $total_sales_result = mysqli_query($conn, $total_sales_query);
    $total_sales = mysqli_fetch_assoc($total_sales_result);

    $total_revenue_query = "SELECT SUM(amount) as total_revenue FROM transactions WHERE status = 'success' AND type NOT IN ('wallet topup', 'wallet deduction')AND DATE(created_at) = CURDATE()";
    $total_revenue_result = mysqli_query($conn, $total_revenue_query);
    $total_revenue = mysqli_fetch_assoc($total_revenue_result);

    $total_topup_query = "SELECT SUM(amount) as total_topup FROM transactions WHERE status = 'success' AND type = 'wallet topup' AND DATE(created_at) = CURDATE()";
    $total_topup_result = mysqli_query($conn, $total_topup_query);
    $total_topup = mysqli_fetch_assoc($total_topup_result);

    $wallet_deduction_query = "SELECT SUM(amount) as total_topup FROM transactions WHERE status = 'success' AND type = 'Wallet deduction' AND DATE(created_at) = CURDATE()";
    $wallet_deduction_result = mysqli_query($conn, $wallet_deduction_query);
    $wallet_deduction = mysqli_fetch_assoc($wallet_deduction_result);

	
	$wallet_profit_query = "SELECT SUM(wallet_profit) AS total_profit FROM transactions WHERE status = 'SUCCESS' AND DATE(created_at) = CURDATE()";
    $wallet_profit_result = mysqli_query($conn, $wallet_profit_query);
        ...
        if ($wallet_profit_result) {
            ...
            $wallet_profit = $row['total_profit'];
        } else {
            // Handle query error
            $wallet_profit = 0;
        }

    $topup = $total_topup['total_topup'] ?? 0.00;
    $deduction = $wallet_deduction['total_topup'] ?? 0.00;
  ...
  // Query for top products
    ...
    while ($trend = mysqli_fetch_assoc($sales_trends_result)) {
     ...
     // Query to get customer discounts
    $cust_discount_query = "SELECT SUM(cust_discount) as total_discount FROM transactions WHERE status = 'success' AND DATE(created_at) = CURDATE()";
    $cust_discount_result = mysqli_query($conn, $cust_discount_query);
    $cust_discount_data = mysqli_fetch_assoc($cust_discount_result);
    $cust_discount = $cust_discount_data['total_discount'] ?? 0.00;
    ...
    <div class="container py-2">
        ...
        <div class="row g-3">
          ...
          <div class="col-lg-4">
              <div class="card mb-3 shadow">
                <div class="card-body bg-light text-dark text-center">
                    <h2><span class="text-primary">&#8358;<?php echo number_format($cust_discount, 2); ?></span></h2>
                    <hr>
         ...
         <div class="col-lg-4">
            <div class="card mb-3 shadow">
            <div class="card-body bg-light text-dark text-center">
              <h2><span class="text-primary">&#8358;<?php echo number_format($wallet, 2); ?></span></h2>
              <hr>
              <h5>Daily Wallet Funding</h5>
            </div>
            </div>
          </div>
		 ...
		 <div class="col-lg-4">
            <div class="card mb-3 shadow">
            <div class="card-body bg-light text-dark text-center">
              <h2><span class="text-primary">&#8358;<?php echo number_format($wallet_profit, 2); ?></span></h2>
              <hr>
              <h5>Daily Wallet Profit</h5>
            </div>
            </div>
          </div>
