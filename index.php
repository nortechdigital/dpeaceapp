...
if (isset($_SESSION['user_id'])) {
  ...
  // Fetch user status and login_status
  $query = "SELECT status, login_status FROM users WHERE id = ?";
  ...
  $stmt->execute();
  $stmt->bind_result($status, $login_status);
  $stmt->fetch();
  $stmt->close();
  
  // If login_status is not 'active', log the user out
  if ($login_status !== 'active') {
      header("Location: ./_/ac_logout.php");
      exit();
  }
}
...
<head>
  ...
  <meta name="description" content="Earn 0.3% to 10% Bonus on Select Subscriptions and Bill Payments!">
  <link rel="canonical" href="https://dpeaceapp.com/" />
...
?>
  ...
  <nav class="navbar navbar-expand-lg bg-white">
    <div class="container">
      ...
      <?php if ($pg !== 'home'): ?>
      <h3 class="text-primary text-center flex-grow-1 w-100">Wallet Balance: <span class="h1" id="">&#8358;<?php echo number_format($balance, 2); ?></span></h3>
      <?php endif; ?>
...
if (typeof(Worker) !== "undefined") {
    ...
    function startWorker() {
        ...
        worker.onmessage = function(e) {
            ...
            if (e.data.result === 'success') {
                console.log('History data loaded at ' + e.data.timestamp, e.data.data);
            	document.getElementById('bal').innerHTML = e.data.data;
            	document.getElementById('bala').innerHTML = e.data.data;
                // Process the data as needed
            } else if (e.data.result === 'error') {
                console.error('Error at ' + e.data.timestamp + ':', e.data.error);
            }
        };
    ...
    // Start the worker when page loads
    startWorker();
    
    // Optionally stop when page unloads
    window.addEventListener('beforeunload', function() {
        stopWorker();
    });
    
} else {
    console.error("Sorry, your browser doesn't support Web Workers.");
}
</script>
<?php endif; ?>
