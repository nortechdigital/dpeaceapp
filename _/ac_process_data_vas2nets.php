...
if ($responseData['data']['status'] === 'Success') {
    ...
    if (!empty($email)) {
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = 'support@dpeaceapp.com';      // Your Zoho email here
            $mail->Password   = 'login@dpeaceSupport1';   // Your Zoho email password/app password here
            ...
            // Content
            $mail->isHTML(true);
            $mail->Subject = 'DPeace App Receipt - Data Subscription';
            $mail->Body    = '
                <html>
                <head>
                    <title>Data Subscription - Receipt</title>
                    <style>
...
$insert_stmt = $conn->prepare("
    ...
    (user_id, fullname, phone_number, product_description, amount, status, transaction_ref, type, profit, detail, cust_discount) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
");
