...
try {
    $discount = test_input($_POST['amount_to_pay']);
...
try {
    ...
    $amount = test_input($_POST['price']);
    $billerId = test_input($_POST['biller_id']);
    $phone = test_input($_POST['phone_number'] ?? '');
    $ported_number = test_input($_POST['ported_number']);
    $plan = test_input($_POST['plan']) ?? '';
    $plan = strstr($plan, ' (', true);
    ...
    $description = "Purchase of $billerId-DATA $plan";
    $detail = "$billerId-DATA";
    ...
    ('$relationId', '$fullname', '$phone', '$outOrderNo', '$description', '$discount', '$status', '0', 'Data Purchase', '$detail', '$cust_discount', '$current_balance', '$new_balance')";
    ...
    $item = PalmPayBillerService::queryItems('data', $billerId);
